...
class StartPage:
    ...
    def initialization(self):
		...
		PRIMARY KEY (id),
		FOREIGN KEY (gid) REFERENCES test_goods(id),
		FOREIGN KEY (uid) REFERENCES test_user(id)
		);

        CREATE INDEX test_order_uid ON test_order USING HASH (uid);
		"""
    ...
    def create_trigger1(self):
		...
		BEGIN
		  IF TG_OP = 'INSERT' THEN
			UPDATE test_goods set price = (price - NEW.money) WHERE id = new.gid;
			RETURN NEW;
		  END IF;
		  RETURN NEW;
		END;
    ...
    def create_trigger2(self):
        ...
        BEGIN
          IF TG_OP = 'DELETE' THEN
            UPDATE test_goods set price = (price + OLD.money) WHERE id = OLD.gid;
            RETURN OLD;
          END IF;
          RETURN OLD;
        END;
    ...
    def create_function1(self):
		...
		$$
		BEGIN
			DELETE FROM test_goods WHERE id = p_商品id;
		END;
		$$
		LANGUAGE 'plpgsql';
		"""
    ...
    def create_function2(self):
		...
		AS $$
		BEGIN
			return query SELECT * FROM test_goods WHERE name = p_name;
		END;
		$$;
		"""
...
class AdminGoods:
    def __init__(self, parent_window):
        ...
        self.tree.column("商品价格", width=int(500/3), anchor='center')
        ...
        if str(self.var_id.get()) in self.id:
            ...
            if self.var_id.get() != '' and self.var_name.get() != '' and self.var_price.get() != '':
                ...
                db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33",
                                      port=26000)
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "INSERT INTO test_goods(id, name, price) \
				       VALUES ('%s', '%s', '%s')" % \
    ...
    def updata_row(self):
        ...
        if res == True:
            ...
            db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "UPDATE test_goods SET id = '%s', name = '%s', price = '%s'where id = '%s'" % (
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql ="""
            DELETE FROM test_order WHERE gid = '%s';
...
class AdminUser:
    def __init__(self, parent_window):
        ...
        db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM test_user"  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
    ...
    def put_data(self):
        ...
        db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM test_user where NAME = '%s'" % (self.chaxun.get())
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
        ...
        if str(self.var_id.get()) in self.id:
            ...
            if self.var_id.get() != '' and self.var_passwd.get() != '' and self.var_name.get() != '' and self.var_number.get() != '' and self.var_addr.get() != '' and self.var_type.get() != '':
                ...
                db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33",
                                      port=26000)
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "INSERT INTO test_user(id, passwd, name, number, addr, type) \
				       VALUES ('%s', '%s', '%s', '%s', '%s', '%s')" % \
    ...
    def updata_row(self):
        ...
        if res == True:
            ...
            db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "UPDATE test_user SET id = '%s', passwd = '%s', name = '%s', number = '%s', addr = '%s', type = '%s' \
				where id = '%s'" % (
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "DELETE FROM test_user WHERE id = '%s'" % (self.row_info[0])  # SQL 插入语句
            try:
                cursor.execute(sql)  # 执行sql语句
                db.commit()  # 提交到数据库执行
                messagebox.showinfo('提示！', '删除成功！')
...
class AdminOrder:
    def __init__(self, parent_window):
        ...
        # 定义下方中心列表区域
        self.columns = ("订单id", "商品id", "用户id", "订单创建时间", "交易金额")
        self.tree = ttk.Treeview(self.frame_center, show="headings", height=18, columns=self.columns)
        self.vbar = ttk.Scrollbar(self.frame_center, orient=VERTICAL, command=self.tree.yview)
        # 定义树形结构与滚动条
        self.tree.configure(yscrollcommand=self.vbar.set)
        ...
        db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM test_order LIMIT 500"  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
        ...
        self.right_top_money_label = Label(self.frame_left_top, text="交易金额:", font=('Verdana', 15))
    ...
    def search_data(self):
        ...
        db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = f"SELECT * FROM test_order LIMIT {self.limit} OFFSET {self.page * self.limit}"  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
    ...
    def put_data(self):
        ...
        sql = "SELECT * FROM test_order where " + condition
        print(sql)
        try:
            # 执行SQL语句
            cursor.execute(sql)
        ...
        if str(self.var_id.get()) in self.id:
            ...
            if self.var_id.get() != '' and self.var_gid.get() != '' and self.var_uid.get() != '' and self.var_buy_time.get() != '' and self.var_money.get() != '':
                ...
                db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33",
                                      port=26000)
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "INSERT INTO test_order(id, gid, uid, buy_time, money) \
				       VALUES ('%s', '%s', '%s', '%s', '%s')" % \
    ...
    def updata_row(self):
        ...
        if res == True:
            ...
            db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "UPDATE test_order SET id = '%s', gid = '%s', uid = '%s', buy_time = '%s', money = '%s' \
				where id = '%s'" % (
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "DELETE FROM test_order WHERE id = '%s'" % (self.row_info[0])  # SQL 插入语句
            try:
                cursor.execute(sql)  # 执行sql语句
                db.commit()  # 提交到数据库执行
                messagebox.showinfo('提示！', '删除成功！')
...
class UserInfo:
    def __init__(self, parent_window, login_id):
        ...
        db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM test_user WHERE id = '%s'" % (login_id)  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            print(self.var_id.get())
            sql = """
            DELETE FROM test_order WHERE uid = '%s';
            DELETE FROM test_user WHERE id = '%s';
...
class UserOrder:
    def __init__(self, parent_window, login_id):
        ...
        db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM test_order WHERE uid = '%s' LIMIT 500" % (login_id)  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
    ...
    def search_data(self, login_id):
        ...
        db = psycopg2.connect(database="db_rzy", user="rzy", password="Rzy021006", host="1.92.80.33", port=26000)
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM test_order WHERE uid = %s LIMIT %d OFFSET %d" % ("\'" + login_id + "\'", self.limit, self.limit * self.page)  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
